...
use App\Filament\Resources\DegreeResource\Pages\CreateDegree;
use App\Filament\Resources\DegreeResource\Pages\EditDegree;
use App\Filament\Resources\DegreeResource\Pages\ListDegrees;
...
beforeEach(function () {
    $this->module = 'degrees';
    $this->module_singular = 'degree';
    $this->class = Degree::class;
    $this->listclass = ListDegrees::class;
    $this->createclass = CreateDegree::class;
    $this->editclass = EditDegree::class;
});
...
it('can render add page if user have add privileges', function () {
    ...
    $response = $this->actingAs(User::where('privilege_id', 6)->first())->withSession(['banned' => false])->get("room/{$this->module}/create");
    $response->assertStatus(200);
    $response->assertSee('Create ' . ucfirst($this->module_singular));
});
