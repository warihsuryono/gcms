...
beforeEach(function () {
    ...
    $this->module_singular = 'employee';
    $this->class = Employee::class;
    $this->listclass = ListEmployees::class;
    $this->createclass = CreateEmployee::class;
    $this->editclass = EditEmployee::class;
});
...
it("can save new data", function () {
    ...
    Livewire::actingAs(User::where('privilege_id', 6)->first())
        ...
        ->fillForm(
            [
                'user_id' => User::where('privilege_id', 6)->first()->id,
                'leader_user_id' => 1,
                'name' => fake('id')->name(),
                'phone' => substr(fake('id')->phoneNumber(), 0, 29),
                'pic_phone' => substr(fake('id')->phoneNumber(), 0, 29),
                'gender' => 'male',
