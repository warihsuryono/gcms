...
use App\Filament\Resources\ItemCategoryResource\Pages\CreateItemCategory;
use App\Filament\Resources\ItemCategoryResource\Pages\EditItemCategory;
use App\Filament\Resources\ItemCategoryResource\Pages\ListItemCategories;
...
beforeEach(function () {
    $this->module = 'item-categories';
    $this->module_singular = 'item-category';
    $this->class = ItemCategory::class;
    $this->listclass = ListItemCategories::class;
    $this->createclass = CreateItemCategory::class;
    $this->editclass = EditItemCategory::class;
});
...
it('can render add page if user have add privileges', function () {
    ...
    $response = $this->actingAs(User::where('privilege_id', 6)->first())->withSession(['banned' => false])->get("room/{$this->module}/create");
    $response->assertStatus(200);
    $response->assertSee('Create Item Category');
});
...
it('can render edit page if user have edit privileges', function () {
    ...
    $response = $this->actingAs(User::where('privilege_id', 6)->first())->withSession(['banned' => false])->get("room/{$this->module}/{$foredit->id}/edit");
    $response->assertStatus(200);
    $response->assertSee('Edit Item Category');
});
