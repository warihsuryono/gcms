...
use App\Filament\Resources\MarriageStatusResource\Pages\CreateMarriageStatus;
use App\Filament\Resources\MarriageStatusResource\Pages\EditMarriageStatus;
use App\Filament\Resources\MarriageStatusResource\Pages\ListMarriageStatuses;
...
beforeEach(function () {
    $this->module = 'marriage-statuses';
    $this->module_singular = 'marriage-status';
    $this->class = MarriageStatus::class;
    $this->listclass = ListMarriageStatuses::class;
    $this->createclass = CreateMarriageStatus::class;
    $this->editclass = EditMarriageStatus::class;
});
...
it('can render add page if user have add privileges', function () {
    ...
    $response = $this->actingAs(User::where('privilege_id', 6)->first())->withSession(['banned' => false])->get("room/{$this->module}");
    $response->assertStatus(200);
    $response->assertSee("New marriage status");
    $response = $this->actingAs(User::where('privilege_id', 6)->first())->withSession(['banned' => false])->get("room/{$this->module}/create");
    $response->assertStatus(200);
    $response->assertSee('Create Marriage Status');
});
...
it('can render edit page if user have edit privileges', function () {
    ...
    $response = $this->actingAs(User::where('privilege_id', 6)->first())->withSession(['banned' => false])->get("room/{$this->module}/{$foredit->id}/edit");
    $response->assertStatus(200);
    $response->assertSee('Edit Marriage Status');
});
