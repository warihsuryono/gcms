...
use App\Filament\Resources\PrivilegeResource\Pages\CreatePrivilege;
use App\Filament\Resources\PrivilegeResource\Pages\EditPrivilege;
use App\Filament\Resources\PrivilegeResource\Pages\ListPrivileges;
use App\Models\menu;
use App\Models\User;
...
beforeEach(function () {
    $this->module = 'privileges';
    $this->module_singular = 'privilege';
    $this->class = Privilege::class;
    $this->listclass = ListPrivileges::class;
    $this->createclass = CreatePrivilege::class;
    $this->editclass = EditPrivilege::class;
});
...
it('can render add page if user have add privileges', function () {
    ...
    $response = $this->actingAs(User::where('privilege_id', 6)->first())->withSession(['banned' => false])->get("room/{$this->module}");
    $response->assertStatus(200);
    $response->assertSee("New privilege");
    $response = $this->actingAs(User::where('privilege_id', 6)->first())->withSession(['banned' => false])->get("room/{$this->module}/create");
    $response->assertStatus(200);
    $response->assertSee('Create Privilege');
});
...
it('can render edit page if user have edit privileges', function () {
    ...
    $response = $this->actingAs(User::where('privilege_id', 6)->first())->withSession(['banned' => false])->get("room/{$this->module}/{$foredit->id}/edit");
    $response->assertStatus(200);
    $response->assertSee('Edit Privilege');
});
